...
Inductive step : tm -> tm -> Prop :=
  ...
  (* pairs *)
  | ST_Pair1 : forall a a' b,
      a --> a' ->
     <{ (a,b) }> --> <{ (a',b) }>
  | ST_Pair2 : forall v b b',
      value v ->
      b --> b' ->
     <{ (v,b) }> --> <{ (v,b') }>
  | ST_Fst : forall t t',
...
Proof with eauto.
  intros t T1 T2 HT Hval.
  inversion HT; subst; try solve_by_inverts 2...
Qed.
*)
...
Theorem progress : forall t T, 
     has_type <{empty}> t T (* <{ empty |-- t \in T }> *) ->
...
Proof with eauto.
  ...
  - (* t.fst *)
   destruct IHHt...
   + right... eapply canonical_forms_pair in H as H'...
 ...
 - (* t.snd *)
  ...
  - right. 
    intuition.
    + apply canonical_forms_fun in Ht... destruct Ht. destruct H. subst...
    + destruct H0...
Qed.
...
Proof.
  unfold includedin.
  intros A m m' x vx H x0 v0 H'.
  destruct (eqb_spec x x0).
  + rewrite e in *.
    rewrite update_eq. rewrite update_eq in H'. 
    apply H'.
  + rewrite update_neq; auto; rewrite update_neq in H'.
    * apply H. apply H'.
    * apply n.
Qed.
...
Proof with eauto.
  ...
  (* Complete the proof. *)
   - destruct (eqb_spec x s); subst... 
     ...
     + eapply T_Let... rewrite update_permute in H6...
Qed.
...
Proof with eauto.
  ...
  (* fst and snd *)
  - intuition. inversion HT;subst...
  - intuition. inversion HT;subst...
  
  (* let *)
  - clear IHHT2 IHHT1. Check T_Let.
    apply (T_Let <{empty}> x0 t1 t2 T1 T2 HT1 ) in HT2.
    inversion HT2;subst...
